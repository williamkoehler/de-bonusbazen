...
pub struct Jobs {
    _job_scheduler: tokio_cron_scheduler::JobScheduler,
        ...
        {
            {
                ...
                tokio::spawn(async move {
                    let last_ah_refresh =
                        ah_jobs.ah_manager.last_refresh().await.unwrap_or_default();
                    ...
                    if needs_ah_refresh {
                        tokio::spawn(async move {
                            if let Err(err) = ah_jobs.update_ah_products_job().await {
